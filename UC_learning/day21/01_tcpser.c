...
int main(void){
    //创建套接字
    printf("服务器:创建套接字\n");
    int sockfd = socket(AF_INET,SOCK_STREAM,0); //sockfd是侦听套机字
    if(sockfd == -1){
        perror("socket");
        return -1;
    }
    //准备地址结构,代表服务器身份
    printf("服务器:组织服务器地址结构\n");
    struct sockaddr_in ser;
    ser.sin_family = AF_INET;
    ser.sin_port = htons(8989);//字节序转换保证协议栈拿到正确的端口号 //8989：服务器端口
    //ser.sin_addr.s_addr = inet_addr("192.168.222.128"); //服务器的IP地址
    ser.sin_addr.s_addr = INADDR_ANY;//接受任意IP地址到来的数据
    
    //绑定套接字和地址结构
    printf("服务器:绑定地址结构\n");
    if(bind(sockfd,(struct sockaddr*)&ser,sizeof(ser)) == -1){ //泛化类型sockaddr，类似void*，sockaddr_un*、sockaddr_in*都可以转换为sockaddr*
        perror("bind");
        return -1;
    }
    //开启侦听 
    printf("服务器:启动侦听\n");
    if(listen(sockfd,1024) == -1){ //1024：等待连接请求有多少个，不少于1024个
        perror("listen");
        return -1;
    }
    //等待并接受客户端的连接请求
    ...
    int conn = accept(sockfd,(struct sockaddr*)&cli,&len);  //conn是通信套接字
    if(conn == -1){
        perror("accept");
        return -1;
    }
...
//仅一次通信
/*
 (base) 176x212x32x15:day22 wyl$ gcc 01_tcpser.c -o ser
 (base) 176x212x32x15:day21 wyl$ ./ser
 服务器:创建套接字
 服务器:组织服务器地址结构
 服务器:绑定地址结构
 服务器:启动侦听
 服务器:等待客户端的连接
 服务器:接受到176.212.32.15:50749的客户端连接
 服务器:业务处理
 服务器:关闭套接字
 */
/*
 (base) 176x212x32x15:day22 wyl$ gcc 01_tcpcli.c -o cli
 (base) 176x212x32x15:day21 wyl$ ./cli
 客户端:创建套接字
 客户端:组织服务器的地址结构
 客户端:向服务器发起连接请求
 客户端:业务处理
